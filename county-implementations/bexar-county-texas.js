...
class BexarCountyTexasScraper extends DeedScraper {
  ...
  async searchBCAD(address) {
    ...
    try {
      ...
      // Step 3: Execute reCAPTCHA before submitting (the site uses the same key for form submission)
      this.log('🔍 Executing reCAPTCHA for form submission...');

      try {
        // Use the same site key from the initial load
        const siteKey = await this.page.evaluate(() => {
          // Check if grecaptcha is loaded
          if (typeof grecaptcha !== 'undefined' && grecaptcha.execute) {
            // Try to get the site key from the page
            const scripts = Array.from(document.querySelectorAll('script'));
            for (const script of scripts) {
        ...
        if (siteKey) {
          this.log(`🔑 Found reCAPTCHA site key for form: ${siteKey.substring(0, 20)}...`);

          // Execute grecaptcha to get a new token
          const formCaptchaToken = await this.page.evaluate(async (key) => {
            if (typeof grecaptcha !== 'undefined' && grecaptcha.execute) {
              try {
          ...
          if (formCaptchaToken) {
            this.log('✅ Got reCAPTCHA token from grecaptcha.execute');

            // Inject into form
            await this.page.evaluate((token) => {
              const input = document.querySelector('input[name="recaptchaToken"]');
              if (input) {
                input.value = token;
              }
            }, formCaptchaToken);
          } else {
            this.log('⚠️ Could not execute grecaptcha, form may be blocked');
          }
        } else {
          this.log('⚠️ reCAPTCHA not loaded on page');
        }
      } catch (error) {
        this.log(`⚠️ reCAPTCHA execution error: ${error.message}`);
      }
      ...
      if (this.verbose) {
        this.log('📄 Page after search:');
        this.log(`   URL: ${pageInfo.url}`);
        this.log(`   Tables: ${pageInfo.tableCount}, Rows: ${pageInfo.rowCount}, Links: ${pageInfo.linkCount}`);
        this.log(`   Body text: ${pageInfo.bodySnippet.substring(0, 200)}...`);
      }
