/**
 * Forsyth County, North Carolina - Deed Scraper Implementation
 *
 * County Resources:
 * - Property Records: https://lrcpwa.ncptscloud.com/forsyth/
 *
...
class ForsythCountyNorthCarolinaScraper extends DeedScraper {
  constructor(options = {}) {
    super(options);
    this.county = 'Forsyth';
    this.state = 'NC';
   ...
   * Example: "3170 Butterfield Dr" -> { streetNumber: "3170", streetName: "Butterfield" }
   */
  parseAddress(address) {
    ...
    cleaned = cleaned.replace(/\b(NC|North Carolina|Forsyth)\b/gi, '');
  ...
  async searchProperty(streetNumber, streetName) {
    ...
    try {
      this.log('🌐 Navigating to Forsyth County Property Search...');
      await this.page.goto('https://lrcpwa.ncptscloud.com/forsyth/', {
      ...
      // First, get the parcel link info - look for numeric link
      const parcelLinkInfo = await this.page.evaluate(() => {
        const allLinks = Array.from(document.querySelectorAll('a'));

        // Look for any link that is purely numeric (parcel number)
        for (const link of allLinks) {
          const text = link.textContent.trim();
          const href = link.href;

          // Look for any link that is purely numeric or has parcel number pattern
...
module.exports = ForsythCountyNorthCarolinaScraper;
