...
async function uploadAttorneys() {
  try {
    ...
    const attorneyDataPath = path.join(__dirname, 'attorney-data', 'california-attorneys-supplement-11.json');
    ...
    const reportPath = path.join(__dirname, 'attorney-data', 'upload-report-11-supplement.json');
    const report = {
    ...
    if (successful.length > 0) {
      const csvPath = path.join(__dirname, 'attorney-data', 'uploaded-11-supplement.csv');
